...
def login_webcam(face_cascade,nn,labels):
    print('Recording..Look in the camera')
    ...
    while True:
        ...
        for (x, y, w, h) in faces:
            ...
            # print(id_,conf)
            if conf >= 4 and conf <= 85:
                if not labels[id_] in name.keys(): 
                    name[labels[id_]] = 1
                else:
                    name[labels[id_]] += 1
                # cv2.putText(img, labels[id_]+'('+str(round(conf,4))+')', (x-10, y-10), cv2.FONT_HERSHEY_SIMPLEX, 1, (102,255,102), 2, cv2.LINE_AA)
            cv2.rectangle(img, (x, y), (x+w,y+h), (255,0,0), 3)
        cv2.imshow('phncam_shot', img)
        if cv2.waitKey(1)== ord('q') or max(name.values()) > 5 :
            break
    cv2.destroyAllWindows()
    return list(dict(sorted(name.items(),key = lambda x:x[1])).items())[-1]
